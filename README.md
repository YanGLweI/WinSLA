...
安装脚本执行以下操作：
1. 复制 `DualAuthCP.dll` 到 `%SystemRoot%\System32\winsla\`
2. 注册 Credential Provider CLSID 到注册表
3. 安装并启动 `WinSLA Service` Windows 服务

### 开发模式测试

```powershell
# 独立模式运行服务（非 Windows Service 方式）
.\target\release\winsla-service.exe

# 服务将监听 Named Pipe: \\.\pipe\winsla-auth-pipe
```
...
cargo test -p winsla-management
```
